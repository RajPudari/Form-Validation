...
const phoneVerify = (number) => {
  const regex = /^(?!123456789)[2-9]\d{2}[2-9]\d{2}\d{4}$/;
...
passwordInput.addEventListener("input", () => {
  ...
  if(passwordInput.value.includes(firstNameInput.value)){
    errorUpdate(passwordInput, passwordError);
    alert("Error: password contain username password is not strong") /// password cant we username
  } 
